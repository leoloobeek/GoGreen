...
func AESEncrypt(key, text []byte) (string, string, error) {
	...
	if len(plaintext)%aes.BlockSize != 0 {
		return "", "", errors.New("[!] AESEncrypt: plaintext is not a multiple of the block size")
	}
...
func pkcs7Pad(b []byte) ([]byte, error) {
	if aes.BlockSize <= 0 {
		return nil, errors.New("[!] pkcs7Pad: invalid blocksize")
	}
	if b == nil || len(b) == 0 {
		return nil, errors.New("[!] pkcs7Pad: invalid PKCS7 data (empty or not padded)")
	}
